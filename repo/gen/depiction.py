...
tweaks = [
    {
        ...
        "title": "haoniu",
        "min_ios": "14.0",
        "description": "<p>An awesome MobileSubstrate tweak🌹🌹🌹🌹🌹🌹🌹!</p>",
        "changes": [
            ["0.0.1-1", "🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹🌹(Rootless-only) Fixed preference not working"],
        ]
    }
]
...
for entry in tweaks:
    ...
    screenshot_objects = list(filter(None, list(map(
        lambda e: None if e.name.startswith('.') else {
            "url": f"https://takedwind.github.io/repo/screenshots/{file}/{e.name}",
    ...
    with open(os.path.join(templates_dir, "index.json")) as json_file:
        ...
        if featured_as_banner and 'headerImage' not in entry:
            data['headerImage'] = f"https://takedwind.github.io/repo/assets/{file}.png"
        tabs = data["tabs"]
        VIEWS = None
